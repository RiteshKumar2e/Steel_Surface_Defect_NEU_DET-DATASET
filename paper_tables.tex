...
\begin{table}[t]
\centering
\caption{Ground-truth box statistics per class. Median box area is expressed as a percentage of the image. Only pitted surface is annotated at near full-frame extent; the remaining classes are small and multiple. This distribution explains the CAM-based localization results of Table~\ref{tab:localization}.}
\label{tab:boxsize}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccc}
\toprule
...
\midrule
Modules & MobileNetV2 baseline & 99.21 & 98.33 \\
 & + FPN & 99.37 & 100.00 \\
 & + AMFF & 99.13 & 100.00 \\
 & + CSAF & 99.29 & 100.00 \\
...
\midrule
\multicolumn{2}{l}{\emph{CAM-based localization}} \\
AP$_{50}$ & 9.71\,$^{\dagger}$ \\
AP$_{75}$ & 3.62\,$^{\dagger}$ \\
...
\midrule
MobileNetV2 baseline & -- & -- & -- & -- & 2{,}423{,}110 & 98.61 & 7.60 & 3.45 \\
+ FPN & \checkmark & -- & -- & -- & 3{,}196{,}102 & 98.89 & 3.94 & 1.48 \\
+ AMFF & \checkmark & \checkmark & -- & -- & 3{,}358{,}111 & \texttt{[RUN]} & \texttt{[RUN]} & \texttt{[RUN]} \\
+ CSAF & \checkmark & \checkmark & \checkmark & -- & 3{,}358{,}687 & \texttt{[RUN]} & \texttt{[RUN]} & \texttt{[RUN]} \\
Full model & \checkmark & \checkmark & \checkmark & \checkmark & 3{,}364{,}064 & \texttt{[RUN]} & \texttt{[RUN]} & \texttt{[RUN]} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
CAM-based localization (Grad-CAM) & image labels only & 3{,}364{,}064 & 9.71\,$^{\dagger}$ & 3.62\,$^{\dagger}$ & 4.30\,$^{\dagger}$ \\
\midrule
\textbf{Detection head (ours)} & bounding boxes & 3{,}966{,}821 & 64.77 & 18.89 & 27.20 \\
...
Inclusion & 65.14 \\
Patches & 84.75 \\
Pitted surface & 82.60 \\
Rolled-in scale & 43.31 \\
Scratches & 77.69 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
MobileNetV2 & 2{,}265{,}670 & 97.22 & 97.55 & 97.22 & 97.21 \\
...
ResNet50 & 23{,}600{,}006 & 100.00 & 100.00 & 100.00 & 100.00 \\
\midrule
\textbf{Proposed (full)} & 3{,}364{,}064 & 93.33 & 94.68 & 93.33 & 92.95 \\
\bottomrule
\end{tabular}
\end{table*}
...
\texttt{[cite]} & \texttt{[fill]} & \texttt{[cls/det]} & \texttt{[fill]} & \texttt{[fill]} & \texttt{[fill]} & \texttt{[Y/N]} \\
\midrule
\textbf{Ours} & Proposed detector & detection & 70/10/20 stratified & $200^2$ & 64.77 & --- \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
MobileNetV2 (backbone only) & 2{,}423{,}110 & 270 & \texttt{[RUN]} & \texttt{[RUN]} & 14.7 \\
...
\textbf{Proposed classifier, 128} & 3{,}364{,}064 & 1373 & 12.83 & 211.84 & 4.7 \\
\midrule
